...
class FtntCli(CliNode):
    def __init__(self, ip: str, port: int = 0, user: str = 'admin', passwd: str = '', hostname='None', conn_type=ConnType.TELNET,
                 version: str = '', labels: dict = {}, timeout=CliDefault.timeout):
        super().__init__(ip, port, user, passwd, hostname, conn_type, labels=labels, timeout=timeout)
...
class FosCli(FtntCli):
    ...
    def __init__(self, ip: str, port: int = 0, user: str = 'admin', passwd: str = '', hostname='None', conn_type=ConnType.TELNET,
                 version: str = '', labels: dict = {}, timeout=CliDefault.timeout):
        super().__init__(ip, port, user, passwd, hostname, conn_type, version, labels) # , timeout=timeout)
        self.prompt_resp.update({'(y/n)': 'y', '-More--': '\033[B'})  # e.g. for purge command, down key for --More--
    ...
    def edit(self, param):
        ...
        m = TcFileParser.P_VARIABLE.match(param) or TcFileParser.P_LOCAL_LABEL.match(param)
        if m:
            key = m.group(1)
...
class FpxCli(FosCli):
    def __init__(self, ip: str, port: int = 23, user: str = 'admin', passwd='', hostname='None', conn_type=ConnType.TELNET,

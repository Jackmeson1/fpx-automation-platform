...
class CliNode(spawn, SysObj):
    ...
    def __init__(self, ip='localhost', port=0, user=CliDefault.user, passwd=CliDefault.passwd,
                 conn_type=ConnType.TELNET, labels: dict = {}, enter_str=CliDefault.ENTER, add_enter=True,
                 prompt_resp=None, login_prompt=CliDefault.login_prompt, passwd_prompt=CliDefault.passwd_prompt,
        ...
        SysObj.__init__(self, conn_type, ip, port, user, passwd, labels)
        ...
        self.prompt_resp: dict = prompt_resp or {CliDefault.PROMPT: None}
        # if prompt_resp:
    ...
    def prompt(self, prompt_resp: dict = None, timeout=-1, merge_prompt=True, refresh_result=True) -> bool:
        ...
        else:  # There should be at least on escape prompt or timeout
            p_re = prompt_resp
        try:
            ...
            # TODO: log and report timeout
            pysys_logger.warning('Unexpected timeout when CLI waiting for prompt', exc_info=True)
            return False
        except EOF:
...
#             raise Exception(self.func_name + " failed.")
    ...
    def proc_err(self, msg: str, action: TcFailAction ='continue'):
        self.tc_log(msg)
    ...
    def arg_expect(self, expect_str: str, flag=re.M):
        ...
        parser.add_argument('-a', '--actoin', type=TcFailAction, choices=list(TcFailAction))
    ...
    def exec_cmd(self, cmd, param=None, cmd_enter=True, is_ctrl=False, wait_for_prompt=True):
        ...
        try:
            self._exec_cmd(cmd, timeout, add_enter, prompt, merge_prompt, is_ctrl, wait_for_prompt,
                           tofinds, any_matches, errs, warnings)
        finally:  # Clear tc command parameters after command execution
            self.cmd_params = {}

    def _exec_cmd(self, cmd: str, timeout: int = -1, add_enter: bool = True,
                  prompt_resp: dict = None, add_prompt: bool = False, is_ctrl=False, wait_for_prompt=True,
                  tofinds=None, any_matches=None, tc_errs=None, warnings=None):
        if is_ctrl:
            assert len(cmd) == 1
        ...
        if wait_for_prompt and self.prompt(prompt_resp, timeout, add_prompt):
            if tofinds:
                ...
                if not analyzer.analyze():
                    ...
                    self.tc_log_err(msg_text)
                    raise SysTcFail(msg_text)
                else:
...
class LinuxCli(WorkStationCli):
    def __init__(self, ip='localhost',
                 ...
                 labels: dict = {}):
        super().__init__(ip, port, user, passwd, conn_type=conn_type,
...
class CentosCli(LinuxCli):
    ...
    def su(self):
        ...
        if self.prompt_resp.get(root_prompt) is None:
            ...
            self.prompt_resp = root_p_re        #  put root prompt in the front
...
class UbuntuCli(LinuxCli):
    def __init__(self, ip='localhost',
                 ...
                 is_console=False,
                 labels: dict = {}):
        super().__init__(ip, port, user, passwd,
                         conn_type=conn_type,
                         prompt_resp={r'{}@.*[$#]\s*$'.format(user): None},
                         timeout=timeout, is_console=is_console,
